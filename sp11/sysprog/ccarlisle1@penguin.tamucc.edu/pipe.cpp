...
int main(int argc, char **argv)
{
    ...
    //create a pipe, a semaphore, and a shared memory segment
    //the shared memory segment stores the struct for data on the pipe
    //fork for a parent and child
    //parent
    //set parent to receive input from pipe
    //child
    //set child output to the pipe
    //
...
pipe_t *pipe_open()
{
    ...
    try
    {
        ...
        shmid = shmget(IPC_PRIVATE, SHM_SIZE, 600);
        if(shmid == -1)
            throw 110;
        ...
        semid = semget(IPC_PRIVATE, 2, 700);
        if(semid == -1)
            throw 127;
...
int pipe_close(pipe_t *p, int how)
{
    ...
    try
    {
        ...
        if(how == O_WRONLY)
        {
            ...
            checkForError = shmdt(shmat((*p).shmid,0,NULL));
            if(checkForError == -1)
                throw 322;
