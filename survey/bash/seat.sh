...
checkForDatabase(){
    if [[ -a seats ]]; then
        clear
        echo "Seats file exists"
    else
        clear
        echo "Creating seats file"
        for (( ROW = 1; ROW < 61; ROW++ ));
        do
            if [[ ROW -gt 9 ]]; then
                letter="A"
                echo "$ROW$letter" >> seats
                letter="B"
                echo "$ROW$letter" >> seats
                letter="C"
                echo "$ROW$letter" >> seats
        ...
        if [[ $Option == "list" ]]; then #display seat list
            ...
            if [[ `echo "$FirstName $LastName $seatSub" |
                ...
                do
                    ...
                    #make a string of seat and name
                    seatRes="$seatSub $FirstName $LastName"; 
                    CONFIRM='n'
                    echo -n "Are you sure that you would like to reserve 
                    seat $seatSub 
                    for $FirstName $LastName (y/n): ";
                    read CONFIRM
                    if [[ $CONFIRM == 'y' ]]; then #check for approval
                        #make temp edit using sed
                        cat seats | sed "s/$seatSub/$seatRes/" > tempSeats 
                        #pipe temporary file to clobber seats file
                        `cat tempSeats > seats` 
                        ...
                        if [[ $moreSeats == 'y' ]]; then 
                            ...
                            #check each new seat entered to see if it is bad
                            while [[ badSeat -gt 0 ]] 
                            do
                                if [[ badSeat == 1 ]]; then #if prev seat was bad
                                    badSeat=0 #reset badSeat to a good seat
                                    echo "That seat $nextSeat is taken or 
                                    does not exist"; #show user what was wrong
                                fi
                                echo "Please enter your next seat to reserve. 
                                When finished type 'done' and hit enter"
                                echo -n "Seat: "; read nextSeat

                                #validate input
                                if [[  $nextSeat != "done" && (`echo $nextSeat | 
                                        grep -c "^\([0-5][0-9]\|60\)[A-F]"` != 1 
                                        || `grep -c "^$nextSeat$" seats` != 1) ]]; 
                                        then
    ...
    count=0 #used to keep track of items displayed
        for (( num = 0; num < 7; num++ )); do #for the first digit 0-6
            for (( num2 = 0; num2 < 10; num2++ )); do #and second digit 0-9
                #if row is empty display it
                if [[ `grep -c "^$num$num2[A-F]$" seats` == 6 ]]; then 
                    echo -n  "$num$num2 "
                        count=$[$count+1]
...
searchLastName() {
    ...
    do
        ...
        if [[ `echo $searchName | grep -c '^[a-zA-Z]\+$'` == 1 ]]; then
            #make sure results only show matches with lastname 
            #(exclude firstname matches)
        if [[ `grep "$searchName" seats | 
            grep -c "^.\{3\} \<[a-zA-Z]\+ \<$searchName"` -ge 1 ]]; then
...
searchSeat() {
    ...
    do
        ...
        if [[ `echo "$searchSeat" | grep -c '^\([0-5][0-9]\|60\)[A-F]$'` 
            ...
            echo "Improperly formatted seat. There are rows 1-60 and seats A-F 
                on each row."
            read
        fi
    done
        ...
        #check to see if it's an actual seat
            ...
            if [[ `grep -c "^$seatNum$" seats` == 1 ]]; then
                ...
                if [[ `echo $newFName | grep -c "[a-zA-Z]\?"` == 1 
                    ...
                    if [[ $approval == "y" ]]; then
                        `cat seats | sed "s/$seatNum $name/$seatNum $newName/" 
                            > tempSeats`
                        `cat tempSeats > seats`
                        `rm tempSeats`
                        echo "Seat $seatNum was changed from $name to $newName"; 
                        read
                    else
    ...
    do
        ...
        if [[ `echo $seatNum | grep -c "\([0-5][0-9]\|60\)[A-F]"` == 1 ]]; then
            if [[ `grep -c "$seatNum [a-zA-Z]*.*" seats` == 1 ]]; then
                ...
                echo -n "Where would you like to move this reservation to? "; 
                read newSeat
                if [[ `echo "$newSeat" | 
                        grep -c "\([0-5][0-9]\|60\)[A-F]"` == 1 ]]; then
                    if [[ `grep -c "^$newSeat$"` == 1 ]]; then
                        echo -n "Are you sure that you want to move $name 
                            from $seatNum to $newSeat (y/n): "; 
                            read approval
                        if [[ $approval == "y" ]]; then
                            `cat seats | sed "s/$seatNum $name/$seatNum/" 
                                | sed "s/$newSeat/$newSeat $name" > tempSeats`
                            `cat tempSeats > seats`
                            `rm tempSeats`
...
delete() {
    ...
    header
        ...
        echo -n "Please type in a last name to cancel the reservation: "; 
        read searchName

        while [[ $searchName != "1" ]]
        do
            if [[ `echo "$searchName" | grep -c "\<[a-zA-Z]"` == 1 && 
                `grep -c "$searchName$" seats` -gt 0 ]]; then
                ...
                echo -n "type 'all' if you would like to cancel all of seats 
                matching $searchName: "; 
                read seatNum; 
                echo
                if [[ $seatNum != "all" ]]; then #delete by seat number
                    if [[ `echo $seatNum | grep -c '^\([0-5][0-9]\|60\)[A-F]$'` 
                        ...
                        if [[ $approval == "y" ]]; then
                            `cat seats | sed "s/$seatNum $removedName/$seatNum/" 
                                > tempSeats`
                            `cat tempSeats > seats`
                            `rm tempSeats`
                            echo "Reservation for $removedName has been 
                                cancelled";read
                        else
                            echo "No reservations for $searchName were cancelled";
                            read
                        fi
                    ...
                    if [[ $approval == "y" ]]; then
                        `cat seats | sed "s/ [a-zA-Z][a-zA-Z]* $searchName//" > 
                            tempSeats`
                        `cat tempSeats > seats`
                        `rm tempSeats`
                        echo "Reservations for $searchName have been cancelled";
                        read
                    else
                ...
                echo "ERROR!"
                echo "There must be only one last name using the letters a-z or 
                    no reservations with that name exist"
            fi
            echo "To quit type '1'";echo
            echo -n "Please type in a last name to cancel the reservation: "; 
        read searchName
